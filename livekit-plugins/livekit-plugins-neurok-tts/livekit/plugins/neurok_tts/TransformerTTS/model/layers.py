...
from tf_keras.layers import Activation, Conv1D, Dense, Dropout, Layer, LayerNormalization

from .transformer_utils import positional_encoding


class CNNResNorm(Layer):
    """
    ...
    def __init__(self,
        ...
        self.convolutions = [Conv1D(filters=f,
                                    kernel_size=kernel_size,
                                    padding=padding)
                             for f in filters[:-1]]
        self.inner_activations = [Activation(inner_activation) for _ in range(self.n_layers - 1)]
        self.last_conv = Conv1D(filters=filters[-1],
                                kernel_size=kernel_size,
                                padding=padding)
        self.normalization = LayerNormalization(epsilon=1e-6)
        self.dropout = Dropout(rate=dout_rate)
...
class TransposedCNNResNorm(Layer):
    """
    Module used in attention blocks, after MHA
    """
...
class FFNResNorm(Layer):
    ...
    def __init__(self,
        ...
        self.d1 = Dense(dense_hidden_units, 'relu')
        self.d2 = Dense(model_dim)
        self.dropout = Dropout(dropout_rate)
        self.last_ln = LayerNormalization(epsilon=1e-6)
...
class SelfAttentionResNorm(Layer):
...
class SelfAttentionDenseBlock(Layer):
...
class SelfAttentionConvBlock(Layer):
...
class SelfAttentionBlocks(Layer):
    def __init__(self,
                 model_dim: int,
...
class CrossAttentionResnorm(Layer):
...
class CrossAttentionDenseBlock(Layer):
...
class CrossAttentionBlocks(Layer):
...
class DecoderPrenet(Layer):
    ...
    def __init__(self,
        ...
        self.d1 = Dense(dense_hidden_units,
                        activation='relu')  # (batch_size, seq_len, dense_hidden_units)
        self.d2 = Dense(model_dim, activation='relu')  # (batch_size, seq_len, model_dim)
        self.rate = tf.Variable(dropout_rate, trainable=False)
        self.dropout_1 = Dropout(self.rate)
        self.dropout_2 = Dropout(self.rate)
...
class Postnet(Layer):
    ...
    def __init__(self, mel_channels: int, **kwargs):
        ...
        self.stop_linear = Dense(3)
        self.mel_out = Dense(mel_channels)
...
class StatPredictor(Layer):
    def __init__(self,
        ...
        self.conv_blocks = CNNDropout(filters=conv_filters,
                                      ...
                                      dout_rate=dropout_rate)
        self.linear = Dense(1, activation=dense_activation)
...
class CNNDropout(Layer):
    def __init__(self,
        ...
        self.dropouts = [Dropout(rate=dout_rate) for _ in range(self.n_layers)]
        self.normalization = [LayerNormalization(epsilon=1e-6) for _ in range(self.n_layers)]
