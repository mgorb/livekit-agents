...
from .assets import abbreviations, audible_symbols, last_resort, special_names, units
from .utils import (
    get_string,
    hyphen_phone_number,
    normalize_phrase,
...
def pre_process(sentence, accessibility):
    ...
    sentence = re.sub(r'[a-zA-Z]{2}[0-9]{2}[a-zA-Z0-9]{4}[0-9]{7}([a-zA-Z0-9]?){0,16}',
                      lambda match: '-'.join(match.group()), sentence)
    sentence = pronounce_names(sentence)
    # hyphenate number from word ending e.g. DigiDoc4 -> digi-dokk-4
    sentence = re.sub(r'(^| )\"?({})((,?\"? |\.\"?)|$)'.format('|'.join(map(re.escape, special_names.keys()))),
                      lambda match: match.group().replace(match.group(2), special_names[match.group(2)]), sentence)
    ...
    sentence = re.sub(r'(([A-ZÄÖÜÕŽŠa-zäöüõšž]+\d|\d+[A-ZÄÖÜÕŽŠa-zäöüõšž])[A-ZÄÖÜÕŽŠa-zäöüõšž0-9]*)',
                      lambda match: ', '.join(match.group()) + ',' if len(match.group()) >= 5 else match.group(),
                      sentence)
    ...
    sentence = re.sub(r'([a-zäöüõšž])([A-ZÄÖÜÕŽŠ][a-zäöüõšž]*)', lambda
        match: f'{match.group(1)}-{match.group(2)}' if not match.group() in abbreviations else match.group(), sentence)
    ...
    if accessibility:
        sentence = re.sub(r'((?<=[^A-ZÄÖÜÕŽŠa-zäöüõšž])|^)( ?)([A-ZÄÖÜÕŽŠ])(?=([^A-ZÄÖÜÕŽŠa-zäöüõšž]|$))',
                          lambda match: match.group(2) + 'suur-täht-' + match.group(3).lower(), sentence)
...
def find_conversions(sentence):
    ...
    for i, postag_list in enumerate(text.morph_analysis.partofspeech):
        ...
        if postag in ('O', 'Y') and re.match('^[IVXLCDM]+$', text_lemma) and (
                len(text.morph_analysis.partofspeech) == i + 1 or not text.morph_analysis.partofspeech[i + 1][
                                                                          0] in num_postags):
