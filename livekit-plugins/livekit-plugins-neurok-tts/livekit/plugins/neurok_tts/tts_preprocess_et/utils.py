...
def get_string(text, index, tag):
    ...
    if own_case in ('', '?'):
        ...
        if index < len(text.words) - 2:
            ...
            if re.match(r'(((\d+\.?)|([IVXLCDM]+-?\w*)),?\s)+(ja|või|ja/või|ning|ega|kui)\s((\d)|([IVXLCDM]+))',
                        text.text[text.words[index].start:]):
                is_list = True
...
def pronounce_names(sentence):
    ...
    TODO: Ta peaks tegema algvormiga checki et kas see on seal names.txts olemas ja sealt võtma õige häälduse ja siis sellele lisama käändelõpu
    """
    for word in synthesizer.analyze(sentence):
        # word_lemma = word['analysis'][0]['lemma'].replace('’', '\'')
        checkable_word = word['text'].lower()
        if names[checkable_word]:
            sentence = sentence.replace(word['text'], names[checkable_word]).replace('’', '\'')
    return sentence
